    ...
    <div className="min-h-screen bg-black">
      ...
      <div className="border-b border-zinc-800">
        <div className="max-w-7xl mx-auto px-6 md:px-16 py-4">
          <nav className="flex items-center gap-2 text-sm text-gray-400">
            ...
            <span>/</span>
            <span className="text-orange-400">Specifications</span>
          </nav>
        </div>
      </div>
      ...
      <section className="py-16 px-6 md:px-16">
        <div className="max-w-7xl mx-auto text-center">
          ...
          <h1 className="text-3xl md:text-5xl font-bold text-white mt-3 mb-4">Technical Specifications</h1>
          <p className="text-gray-400 max-w-xl mx-auto text-lg">
            Core technical parameters across all models for easy comparison and selection
          </p>
        </div>
      </section>
      ...
      <section className="pb-24 px-6 md:px-16">
          ...
          <table className="w-full text-sm text-left border-collapse">
            <thead>
              <tr className="border-b border-zinc-700">
                <th className="py-3 px-4 text-gray-400 font-semibold sticky left-0 bg-black z-10 min-w-[160px]">Model</th>
                {productsEn.map((p) => (
                  <th key={p.slug} className="py-3 px-4 text-orange-400 font-semibold whitespace-nowrap">
                    <Link href={`/en/products/${p.slug}`} className="hover:text-orange-300 transition-colors">{p.name}</Link>
                  </th>
                ))}
              </tr>
            </thead>
        ...
        <div className="max-w-7xl mx-auto mt-12 text-center">
          <p className="text-gray-500 text-sm mb-6">Click model name to view full product details</p>
          <Link
            href="/en/products"
